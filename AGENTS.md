# Key-aware diff for CSV/Parquet/Arrow files with git and DVC integration (`tabular-file-diff`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://tabular-file-diff.sociobot.in`. Artifact class: `library-pypi`.
